...
- **activity_intervals** — ground-truth time ranges (`active_gameplay` / `afk` / `menu` / `loading`) via dev hotkeys or API. ML CSV is built **inside the service** with `go run ./cmd/collectdataset`, or **on a schedule** by the API process (`DATASET_AUTOMATION_*` env vars; writes `dataset-windows-YYYY-MM-DD.csv` for **yesterday UTC** under `DATASET_AUTOMATION_OUTPUT_DIR`).
