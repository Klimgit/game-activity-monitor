...
Desktop Client (Go)
  ...
  ├── System tray menu (manual session / activity interval labeling)
...
- **activity_intervals** — ground-truth time ranges (`active_gameplay` / `afk` / `menu` / `loading`) via the desktop client tray menu or API. ML CSV is built **inside the service** with `go run ./cmd/collectdataset` (uses `DATABASE_URL`, not end-user HTTP).
